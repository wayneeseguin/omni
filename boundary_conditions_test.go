...
func TestUnicodeAndSpecialCharacters(t *testing.T) {
	...
	metrics := logger.GetMetrics()
	if metrics.MessagesLogged[LevelInfo] != uint64(len(testCases)) {
		t.Errorf("Expected %d messages logged, got %d", len(testCases), metrics.MessagesLogged[LevelInfo])
	}
}
