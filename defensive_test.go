...
func TestBufferPoolDefensive(t *testing.T) {
	...
	// Create a custom pool with non-buffer objects to test type assertion
	customPool := &BufferPool{
		pool: sync.Pool{
			New: func() interface{} {
				// Return wrong type on purpose
				return "not a buffer"
			},
		},
	}
	...
	buf3 := customPool.Get()
	if buf3 == nil {
		t.Fatal("Got nil buffer when handling wrong type")
	}
	// Should get a new buffer, not the string
	if _, ok := interface{}(buf3).(*bytes.Buffer); !ok {
		t.Fatal("Did not get a buffer back")
	}
}
...
func TestConcurrentMapOperations(t *testing.T) {
	...
	t.Logf("Final custom levels count: %d", len(finalMetrics.MessagesLogged))
}
