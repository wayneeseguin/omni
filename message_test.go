...
func TestProcessSyslogMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.nilConn && !strings.Contains(stderrBuf.String(), "Syslog connection not initialized") {
				t.Errorf("Expected stderr to contain 'Syslog connection not initialized'")
			}
...
func TestMessageFormatting(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create a modified logger with a mock Lock
			modifiedLogger := *logger
			...
			modifiedLogger.processFileMessage(tt.message, dest, &entry, &entrySize)
