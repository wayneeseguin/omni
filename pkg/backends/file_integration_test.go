...
func testDiskFullWithRotation(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if err != nil {
			...
			// If we succeed after a previous error, rotation likely occurred
			if previousErr {
				rotationOccurred = true
				t.Log("Rotation detected: write succeeded after previous failure")
			}
			previousErr = false
		}
	...
	// Verify rotation happened by checking error log for rotation messages
	rotationMessages := 0
	for _, msg := range errorLog {
		if strings.Contains(msg, "Rotated log to") {
			rotationMessages++
		}
	}

	if !rotationOccurred && rotationMessages == 0 {
		t.Error("Expected at least one rotation to occur")
	} else if rotationMessages > 0 {
		t.Logf("Found %d rotation messages in error log", rotationMessages)
	}
