...
func BenchmarkLoggingWithoutPooling(b *testing.B) {
	...
	// Note: mediumBufferPool not available in current implementation
	// Skipping buffer pool test
	...
	// Note: SetFlushInterval not available in current implementation
...
func BenchmarkLoggingWithBatching(b *testing.B) {
	...
	// Note: SetFlushInterval and SetFlushSize not available in current implementation
...
func BenchmarkMessageFormatting(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		// Create a LogMessage for formatting
		msg := LogMessage{
			Level:  LevelInfo,
			Format: "Benchmark message %d with %s",
			Args:   []interface{}{i, "parameters"},
		}
		_, _ = logger.formatMessage(msg)
	}
}
