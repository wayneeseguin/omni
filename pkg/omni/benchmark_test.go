...
func BenchmarkStructuredLogging(b *testing.B) {
	...
	b.Run("StructuredLog", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			// StructuredLog is not available
...
func BenchmarkFormats(b *testing.B) {
	// LogFormat type is not available
	benchmarks := []struct {
		name   string
...
func BenchmarkSampling(b *testing.B) {
	// SamplingStrategy type is not available
	strategies := []struct {
		name     string
...
func BenchmarkAllocations(b *testing.B) {
	...
	b.Run("StructuredMessage", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			// StructuredLog is not available
			logger.InfoWithFields("structured", fields)
		}
		logger.Sync()
	})
}
...
func BenchmarkErrorLogging(b *testing.B) {
	...
	b.Run("WithoutStackTrace", func(b *testing.B) {
		// EnableStackTraces is not available
		// logger.EnableStackTraces(false)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			// ErrorWithError is not available
			logger.Errorf("error occurred: %v", testErr)
		}
		logger.Sync()
	})
