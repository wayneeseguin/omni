...
func TestMessageProcessing(t *testing.T) {
	...
	t.Run("CustomBackend", func(t *testing.T) {
		...
		// AddCustomDestination is not available
...
func TestProcessMessageFormatting(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			formatOpts: FormatOptions{
				IncludeTime:  false,
				IncludeLevel: true,
				LevelFormat:  formatters.LevelFormatName,
			},
			level:         LevelWarn,
		...
		},
		{
			...
			formatOpts: FormatOptions{
				IncludeTime:  false,
				IncludeLevel: true,
				LevelFormat:  formatters.LevelFormatSymbol,
			},
			level:         LevelError,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Set format options
			logger.mu.Lock()
			// Ensure TimeZone is set to avoid panic
			if tt.formatOpts.TimeZone == nil {
				tt.formatOpts.TimeZone = time.UTC
			}
			...
			for _, part := range tt.expectedParts {
				if !bytes.Contains(content, []byte(part)) {
					...
					// Debug: Log the format options
					logger.mu.RLock()
					t.Logf("FormatOptions: %+v", logger.formatOptions)
					logger.mu.RUnlock()
				}
			}
