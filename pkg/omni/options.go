	...
	return func(c *Config) error {
		if level < LevelTrace || level > LevelError {
			return NewOmniError(ErrCodeInvalidLevel, "config", "", nil).
				WithContext("level", fmt.Sprintf("%d", level))
		}
		c.Level = level
		return nil
	}
	...
	return func(c *Config) error {
		if format != FormatText && format != FormatJSON {
			return NewOmniError(ErrCodeInvalidFormat, "config", "", nil).
				WithContext("format", fmt.Sprintf("%d", format))
		}
		c.Format = format
		return nil
	}
...
func WithChannelSize(size int) Option {
	return func(c *Config) error {
		if size <= 0 {
			return NewOmniError(ErrCodeInvalidConfig, "config", "", nil).
				WithContext("channelSize", fmt.Sprintf("%d", size))
		}
		c.ChannelSize = size
		return nil
	}
...
func WithSampling(strategy int, rate float64) Option {
	return func(c *Config) error {
		if rate < 0 || rate > 1 {
			return NewOmniError(ErrCodeInvalidConfig, "config", "", nil).
				WithContext("samplingRate", fmt.Sprintf("%.2f", rate)).
...
func WithMaxAge(duration time.Duration) Option {
	return func(c *Config) error {
		if duration < 0 {
			return NewOmniError(ErrCodeInvalidConfig, "config", "", nil).
				WithContext("maxAge", duration.String()).
...
func WithCleanupInterval(interval time.Duration) Option {
	return func(c *Config) error {
		if interval <= 0 {
			return NewOmniError(ErrCodeInvalidConfig, "config", "", nil).
				WithContext("cleanupInterval", interval.String()).
...
// WithBackendFactory sets a custom backend factory for creating destinations.
// This allows for custom destination implementations beyond the built-in backends.
//
// Parameters:
//   - factory: The backend factory implementation
//
// Returns:
//   - Option: The configuration option
func WithBackendFactory(factory BackendFactory) Option {
	return func(c *Config) error {
		if factory == nil {
			return NewOmniError(ErrCodeInvalidConfig, "config", "", nil).
				WithContext("error", "backend factory cannot be nil")
		}
		c.BackendFactory = factory
		return nil
	}
}

// WithFormatter sets a custom formatter for log output.
// This allows for custom formatting beyond the built-in text and JSON formatters.
//
// Parameters:
//   - formatter: The formatter implementation
//
// Returns:
//   - Option: The configuration option
func WithFormatter(formatter Formatter) Option {
	return func(c *Config) error {
		if formatter == nil {
			return NewOmniError(ErrCodeInvalidConfig, "config", "", nil).
				WithContext("error", "formatter cannot be nil")
		}
