...
func TestMetricsBoundaryConditions(t *testing.T) {
	t.Run("metrics with no messages", func(t *testing.T) {
		...
		if metrics.ChannelUsage != 0 {
			t.Errorf("ChannelUsage = %d, want 0", metrics.ChannelUsage)
		}
		if metrics.MessagesDropped != 0 {
			t.Errorf("MessagesDropped = %d, want 0", metrics.MessagesDropped)
		}
		if len(metrics.MessagesByLevel) != 0 {
			t.Errorf("MessagesByLevel should be empty, got %d entries", len(metrics.MessagesByLevel))
		}
	})
	...
	t.Run("reset metrics multiple times", func(t *testing.T) {
		...
		if metrics.MessagesLogged != 0 {
			t.Errorf("MessagesLogged should be reset, got %d messages", metrics.MessagesLogged)
		}
	})
}
...
func TestFormattingBoundaryConditions(t *testing.T) {
	...
	t.Run("very long field values", func(t *testing.T) {
		...
		logger.InfoWithFields("test", map[string]interface{}{
